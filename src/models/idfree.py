...
class IDFREE(GeneralRecommender):
    def __init__(self, config, dataset):
        ...
        # 新增：动态语义增强的注意力模块
        if self.use_text and self.use_image:
            self.modal_attention = ModalAttention(self.embedding_dim, self.device)
    ...
    def forward(self, training):
        ...
        # 修改：使用动态语义增强机制融合多模态特征
        if self.use_text and self.use_image:
            # 调用注意力模块动态计算文本和视觉模态的权重
            u_embedding = self.modal_attention(u_t_mu, u_v_mu)
            i_embedding = self.modal_attention(i_t_mu, i_v_mu)
        elif self.use_text:
            u_embedding = u_t_mu
            i_embedding = i_t_mu
        elif self.use_image:
    ...
    def sl_loss(self, users, pos_items, neg_items):
        ...
        neg_scores = F.cosine_similarity(users, neg_items, dim=1)
...
class ModalAttention(nn.Module):
    def __init__(self, embedding_dim, device):
        ...
        self.device = device
        # 轻量级注意力网络，计算文本和视觉模态的权重
        self.attn_fc = nn.Linear(2 * embedding_dim, 2)  # 输入为拼接的文本和视觉嵌入，输出为两个模态的权重
        self.to(device)

    def forward(self, text_emb, visual_emb):
        # 拼接文本和视觉嵌入
        combined_emb = torch.cat([text_emb, visual_emb], dim=-1)
        # 计算注意力权重
        attn_weights = F.softmax(self.attn_fc(combined_emb), dim=-1)
        # 按权重融合文本和视觉嵌入
        fused_emb = attn_weights[:, 0].unsqueeze(1) * text_emb + attn_weights[:, 1].unsqueeze(1) * visual_emb
        return fused_emb