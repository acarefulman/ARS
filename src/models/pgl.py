...
class PGL(GeneralRecommender):
    ...
    def global_subgraph_extraction(self, adj):
        ...
        top_s = s[:num_top_bottom]
        bottom_s = s[-num_top_bottom:]
        product_s = top_s * bottom_s
        S = np.diag(product_s)

        # 构建新的稀疏邻接矩阵：注意是 u @ S @ vt（不是 u.T）
        u_k = u[:, :num_top_bottom]  # shape: [n_nodes, k]
        vt_k = vt[:num_top_bottom, :]  # shape: [k, n_nodes]
        product_matrix = u_k @ S @ vt_k  # shape: [n_nodes, n_nodes]

        # 二值化 + 构建稀疏矩阵
        product_matrix = product_matrix * (np.abs(product_matrix) >= 1e-3)
        product_sparse_matrix = sp.csr_matrix(product_matrix)

        # 打印确认尺寸是否匹配
        print("✔ SVD Subgraph shape:", product_sparse_matrix.shape)
