...
class ALIGNREC(GeneralRecommender):
    ...
    def calculate_loss(self, interaction, not_train_ui=False):
        ...
        if self.ui_cosine_loss:
            batch_mf_loss += (1 - F.cosine_similarity(u_g_embeddings, pos_i_g_embeddings,
                                                      dim=-1).mean()) * self.ui_cosine_loss_weight

        pos_ii_batch_sim_mat = build_sim(self.v_feat[pos_items])
        neg_ii_batch_sim_mat = build_sim(self.v_feat[neg_items])

        if self.use_bce:
            ii_sim_loss = self.sim_sigmoid_loss(side_embeds_items[pos_items],
                                                pos_ii_batch_sim_mat) + self.sim_sigmoid_loss(
