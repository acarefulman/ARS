...
Ablation: Disabled Residual-Enhanced Causal Preference Modeling
- Removed residual connections in forward
- Removed gate_v, gate_t, gate_f, gate_image_prefer, gate_text_prefer, gate_fusion_prefer
"""
...
class SMORE_NO_RESIDUAL(GeneralRecommender):
    def __init__(self, config, dataset):
        super(SMORE_NO_RESIDUAL, self).__init__(config, dataset)
        self.sparse = True
    ...
    def forward(self, adj, train=False):
        ...
        image_item_embeds = self.item_id_embedding.weight  # Removed gate_v
        text_item_embeds = self.item_id_embedding.weight   # Removed gate_t
        fusion_item_embeds = self.item_id_embedding.weight # Removed gate_f
        ...
        for _ in range(self.n_ui_layers):
            ...
            ego_embeddings = side_embeddings  # Removed residual connection
            all_embeddings.append(ego_embeddings)
        content_embeds = torch.stack(all_embeddings, dim=1).mean(dim=1)

        def propagate(modality_embeds, adj_matrix, att):
            for _ in range(self.n_layers):
                att = self.att_dropout(torch.sigmoid(att))
                modality_embeds = torch.sparse.mm(adj_matrix, modality_embeds * att)
        ...
        image_prefer = torch.ones_like(content_embeds)  # Removed gate_image_prefer
        text_prefer = torch.ones_like(content_embeds)   # Removed gate_text_prefer
        fusion_prefer = torch.ones_like(content_embeds) # Removed gate_fusion_prefer
