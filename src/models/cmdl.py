...
class CMDL(GeneralRecommender):
    def __init__(self, config, dataset):
        ...
        self.club_zx_zs = CLUBInfoNCECritic(self.embedding_dim, self.embedding_dim, self.critic_hidden_dim,
                                            self.critic_layers, activation='relu')
        self.club_zy_zs = CLUBInfoNCECritic(self.embedding_dim, self.embedding_dim, self.critic_hidden_dim,
                                            self.critic_layers, activation='relu')

        self.infonce_x_zs = InfoNCECritic(self.embedding_dim, self.embedding_dim, self.critic_hidden_dim,
                                          self.critic_layers, activation='relu')
        self.infonce_y_zs = InfoNCECritic(self.embedding_dim, self.embedding_dim, self.critic_hidden_dim,
                                          self.critic_layers, activation='relu')

        self.club_x_ys_cond = CLUBInfoNCECritic(self.embedding_dim + self.embedding_dim, self.embedding_dim,
                                                self.critic_hidden_dim, self.critic_layers, activation='relu')
        self.club_y_xs_cond = CLUBInfoNCECritic(self.embedding_dim + self.embedding_dim, self.embedding_dim,
                                                self.critic_hidden_dim, self.critic_layers, activation='relu')
    ...
    def forward(self, adj, train=False):
        ...
        weight = torch.tensor(
            [generated_weights[:, 0].unsqueeze(dim=1).max(), generated_weights[:, 1].unsqueeze(dim=1).max(),
             generated_weights[:, 2].unsqueeze(dim=1).max()])
        normalized_weight = torch.nn.functional.softmax(weight, dim=0)
        side_embeds = normalized_weight[0] * sep_image_embeds + normalized_weight[1] * sep_text_embeds + \
                      normalized_weight[2] * sep_share_embeds
    ...
    def calculate_loss(self, interaction):
        if len(interaction) < 3:
            raise ValueError("Interaction tensor must contain users, positive items, and negative items")

        users = interaction[0]
        pos_items = interaction[1]
        neg_items = interaction[2]  # Use index 2 instead of self.NEG_ITEM_ID
        ...
        m_batch_mf_loss, m_batch_emb_loss, m_batch_reg_loss = self.bpr_loss(side_embeds_users[users],
                                                                            side_embeds_items[pos_items],
                                                                            side_embeds_items[neg_items])
        cl_loss = self.InfoNCE(side_embeds_items[pos_items], content_items[pos_items], self.info_pos) + self.InfoNCE(
            side_embeds_users[users], content_users[users], self.info_neg)

        high_zx_zs = self.club_zx_zs(self.text_specific[pos_items], self.share[pos_items]) + self.club_zx_zs(
            self.text_specific[neg_items], self.share[neg_items])
        high_zy_zs = self.club_zy_zs(self.image_specific[pos_items], self.share[pos_items]) + self.club_zy_zs(
            self.image_specific[neg_items], self.share[neg_items])

        low_x_zs = self.infonce_x_zs(self.text_embeds[pos_items], self.share[pos_items]) + self.infonce_x_zs(
            self.text_embeds[neg_items], self.share[neg_items])
        low_y_zs = self.infonce_y_zs(self.image_embeds[pos_items], self.share[pos_items]) + self.infonce_y_zs(
            self.image_embeds[neg_items], self.share[neg_items])

        cond_high_x_ys = self.club_x_ys_cond(torch.cat([self.share[pos_items], self.image_embeds[pos_items]], dim=1),
                                             self.text_embeds[pos_items]) + self.club_x_ys_cond(
            torch.cat([self.share[neg_items], self.image_embeds[neg_items]], dim=1), self.text_embeds[neg_items])
        cond_high_y_xs = self.club_y_xs_cond(torch.cat([self.share[pos_items], self.text_embeds[pos_items]], dim=1),
                                             self.image_embeds[pos_items]) + self.club_y_xs_cond(
            torch.cat([self.share[neg_items], self.text_embeds[neg_items]], dim=1), self.image_embeds[neg_items])
    ...
    def learning_loss(self, interaction):
        ...
        low_zx_zs = self.club_zx_zs.learning_loss(self.text_specific[pos_items],
                                                  self.share[pos_items]) + self.club_zx_zs.learning_loss(
            self.text_specific[neg_items], self.share[neg_items])
        low_zy_zs = self.club_zy_zs.learning_loss(self.image_specific[pos_items],
                                                  self.share[pos_items]) + self.club_zy_zs.learning_loss(
            self.image_specific[neg_items], self.share[neg_items])
        cond_low_x_ys = self.club_x_ys_cond.learning_loss(
            torch.cat([self.share[pos_items], self.image_embeds[pos_items]], dim=1),
            self.text_embeds[pos_items]) + self.club_x_ys_cond.learning_loss(
            torch.cat([self.share[neg_items], self.image_embeds[neg_items]], dim=1), self.text_embeds[neg_items])
        cond_low_y_xs = self.club_y_xs_cond.learning_loss(
            torch.cat([self.share[pos_items], self.text_embeds[pos_items]], dim=1),
            self.image_embeds[pos_items]) + self.club_y_xs_cond.learning_loss(
            torch.cat([self.share[neg_items], self.text_embeds[neg_items]], dim=1), self.image_embeds[neg_items])
