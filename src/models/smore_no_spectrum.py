...
Ablation: Disabled Dynamic Modality Quality-Guided Spectral Fusion
- Removed Fourier transform and complex weights in spectrum_convolution
- Fixed modality weights instead of dynamic alpha_v, alpha_t, alpha_f
"""
...
class SMORE_NO_SPECTRUM(GeneralRecommender):
    def __init__(self, config, dataset):
        super(SMORE_NO_SPECTRUM, self).__init__(config, dataset)
        self.sparse = True
    ...
    def spectrum_convolution(self, image_embeds, text_embeds):
        # Disabled spectral fusion: return average of embeddings
        fusion_embeds = (image_embeds + text_embeds) / 2 if image_embeds is not None and text_embeds is not None else image_embeds or text_embeds
        return image_embeds, text_embeds, fusion_embeds
    ...
    def forward(self, adj, train=False):
        ...
        modality_weights = torch.tensor([1/3, 1/3, 1/3]).to(self.device)  # Fixed weights
